...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    const { data: request, error } = await db
      ...
      .select('id')
      .single();
    if (error || !request) throw badRequest('Could not create deletion request', error?.message);
    ...
    await db.from('deletion_requests').update({ status: 'completed', completed_at: new Date().toISOString() }).eq('id', request.id);
